...
import org.devio.hi.ui.item.ChViewHolder

class ChSliderView @JvmOverloads constructor(
    ...
    private val MENU_TEXT_SIZE = applyUnit(TypedValue.COMPLEX_UNIT_DIP, 14f)
    ...
    private val MENU_ITEM_LAYOUT_RES_ID = R.layout.ch_slider_menu_item
    private val CONTENT_ITEM_LAYOUT_RES_ID = R.layout.ch_slider_content_item
    ...
    fun bindMenuView(
        ...
        itemCount :Int,
        onBindView: (ChViewHolder, Int) -> Unit,
        onItemClick: (ChViewHolder, Int) -> Unit){
    ...
    private fun parseMenuItemAttr(attrs: AttributeSet?): MenuItemAttr {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.ChSliderView)

        val menuItemWidth =
            typedArray.getDimensionPixelOffset(R.styleable.ChSliderView_menuItemWidth, MENU_WIDTH)
        val menuItemHeight =
            typedArray.getDimensionPixelOffset(R.styleable.ChSliderView_menuItemHeight, MENU_HEIGHT)
        val menuItemTextSize = typedArray.getDimensionPixelOffset(
            R.styleable.ChSliderView_menuItemTextSize,
            MENU_TEXT_SIZE
        )
        val menuItemSelectTextSize = typedArray.getDimensionPixelOffset(
            R.styleable.ChSliderView_menuItemSelectTextSize,
            MENU_TEXT_SIZE
        )

        val menuItemTextColor =
            typedArray.getColorStateList(R.styleable.ChSliderView_menuItemTextColor)
                ?: generateColorStateList()

        val menuItemIndicator = typedArray.getDrawable(R.styleable.ChSliderView_menuItemIndicator)
            ?: ContextCompat.getDrawable(context, R.drawable.shape_ch_slider_indicator)

        val menuItemBackgroundColor =
            typedArray.getColor(R.styleable.ChSliderView_menuItemBackgroundColor, BG_COLOR_NORMAL)
        val menuItemBackgroundSelectColor = typedArray.getColor(
            R.styleable.ChSliderView_menuItemSelectBackgroundColor,
            BG_COLOR_SELECT
        )
        typedArray.recycle()
        return MenuItemAttr(
    ...
    inner class MenuAdapter(
        val layoutRes: Int,
        val count: Int,
        val onBindView: (ChViewHolder, Int) -> Unit,
        val onItemClick: (ChViewHolder, Int) -> Unit
    ) : RecyclerView.Adapter<ChViewHolder>() {
        //本次选中的item的位置
        ...
        private fun applyItemAttr(position: Int, holder: ChViewHolder) {
            val selected = position == currentSelectIndex
            val titleView: TextView? = holder.itemView.findViewById(R.id.menu_item_title)
            val indicatorView: ImageView? = holder.itemView.findViewById(R.id.menu_item_indicator)
    ...
    inner class ContentAdapter(val layoutRes: Int) : RecyclerView.Adapter<ChViewHolder>() {
        private lateinit var onItemClick: (ChViewHolder, Int) -> Unit
        private lateinit var onBindView: (ChViewHolder, Int) -> Unit
        ...
        override fun onViewAttachedToWindow(holder: ChViewHolder) {
